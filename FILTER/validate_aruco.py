...
psi1_angle = math.radians(150) # 26 degrees between tag's X and true north
...
for image_file_no in range(total_image_files):
    ...
    for rvec, tvec, markerId in zip(rvecs, tvecs, markerIds):
        if markerId == 0:
            ...
            rvec_mat = Rotation.from_rotvec(rvec[0]).as_dcm().T
            flip_mat = np.array([[1,0,0],[0,-1,0],[0,0,-1]])
            camera_pos = psi_mat @ flip_mat @ (tag_position_offset.T + rvec_mat @ (-tvec[0].T))
            camera_pos[1] = camera_pos[1] + 0.44
            tme = float(image_info[image_file_no][0])
            camera_pos_stack_row = np.concatenate((np.array([tme]),camera_pos))
            camera_pos_stack = np.concatenate((camera_pos_stack, [camera_pos_stack_row]), axis=0)
            print('Time {:.3f} camera position is '.format(tme), camera_pos)
    i = i+1

np.savetxt('truth.txt', camera_pos_stack, delimiter=',')
plt.plot(camera_pos_stack[:,0], camera_pos_stack[:,1:4])
plt.show()
