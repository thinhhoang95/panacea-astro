...
def mpu_daq():
    ...
    global hack, t_damp, imu_terminate_flag, imu_start_flag, clock, imu, t_print
...
def cam_daq_3():
    ...
    while not imu_terminate_flag:
        if not imu_start_flag:
            continue
        with camera:
            print('(CAM) Camera capture started')
            camera.capture_sequence(filenames(), use_video_port=True)

# Read keyboard key to end the experiment
def read_keyboard_key():
    global imu_terminate_lock, imu_terminate_flag, tme, accX, accY, accZ, psi, the, phi
    print('(KEY) MPU recording will be effective in 20 seconds')
    while True:
        if time.time() - t_begin > 20:
