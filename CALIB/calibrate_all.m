...
omega_x = IMU0x2Domega(:,4)' * pi/180 - offset_gyro_x*ones(1,total_sample);
omega_y = IMU0x2Domega(:,3)' * pi/180 - offset_gyro_y*ones(1,total_sample);
omega_z = IMU0x2Domega(:,2)' * pi/180 - offset_gyro_z*ones(1,total_sample);
...
figure
plot(s_square);
title('Allan variance of samples (s_quare)')
xlabel('Time sample')
ylabel('Variance')
...
for times_the_var = 1:max_times_the_var
    ...
    % detect intervals whose immobile filter puts out 1
    ...
    qsTime = 1;
    sample_period = 0.01;
    num_samples = qsTime/sample_period; % literally finds all immobile duration whose sample length is less than num_samples
    ...
    % minimization
    selectedAccData = selected_data * 9.78206; % convert g to gravitational acceleration at Ho Chi Minh
    % selectedAccData(:,floor(length(selectedAccData)/2):length(selectedAccData)) = - selectedAccData(:,floor(length(selectedAccData)/2):length(selectedAccData));
    theta_pr = [0, 0, 0, 1, 1, 1, 0, 0, 0];
    % theta_pr = [0, 0, 0, 1.2, 1.2, 1.2, -1, -1, -1];
    % note: the selectedAccData contains all samples to optimize, do we
    % need to take average of them first so that the size of
    % selectedAccData will be the same as QS_time_interval... which
    % basically denotes the "ON" periods (from which time sample to which sample) of the immobile filter
    
    test = accCostFunctLSQNONLIN(theta_pr, selectedAccData);
    ...
    accel_lb = [-0.012,-0.012,-0.012,0.86,0.86,0.86,-0.1,-0.1,-0.1];
    accel_ub = [0.012,0.012,0.012,1.2,1.2,1.2,0.1,0.1,0.1];
...
figure
plot(1:length(selectedAccData),selectedAccData)
title('Data to be optimized')
...
% Redraw the norm vector after calibrated
for i = 1:length(a_xp)
    norm_a(i) = norm(9.78206 * [a_xp(i) a_yp(i) a_zp(i)]);
    a_corrected = estimated_misalignmentMatrix * estimated_scalingMatrix * 9.78206 * [a_xp(i) a_yp(i) a_zp(i)]' - estimated_biasVector;
    norm_ac(i) = norm(a_corrected);
end
...
plot(norm_ac)
hold off
legend('Original norm','Calibrated norm');
%-------------------------------------------------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(filter)
    ...
    elseif flag == 1 && filter(i) == 1
        samples = samples + 1;
    elseif flag == 1 && filter(i) == 0
        if l>1 && QS_time_interval_info_matrix(2,l-1) + 1 ~= start - 1
...
disp('Optimizing gyroscope...');
% Minimizing LSQNONLIN
theta_pr_gyro = [1,0,0,0,1,0,0,0,1];
initial_residual_norm = gyroCostFunctLSQNONLIN(theta_pr_gyro, QS_time_interval_calib_info_matrix, omega_x, omega_y, omega_z);
option = optimset('TolX', 10^-7, 'TolFun' , 10^-6, 'MaxFunEvals', 400);
[theta_pr_gyro, residual_norm] = lsqnonlin(@(theta_pr_gyro) gyroCostFunctLSQNONLIN(theta_pr_gyro, QS_time_interval_calib_info_matrix, omega_x, omega_y, omega_z), theta_pr_gyro, [], [], option);
